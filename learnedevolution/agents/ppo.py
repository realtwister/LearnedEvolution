...
class PPO(object):
    ...
    def _setup_optimizer(self):
        ...
        with tf.name_scope("gradient"):
            trainer = tf.train.AdamOptimizer(learning_rate = self._learning_rate, epsilon=1e-5)
            grads_and_var = trainer.compute_gradients(self.tf_loss, var_list)
            grads, var = zip(*grads_and_var)
            self._rr(tf.reshape(tf.concat([tf.reshape(grad,[-1]) for grad in grads], axis=0),[-1]), 'gradients');

            if True or max_grad_norm is not None:
                grads, _grad_norm = tf.clip_by_global_norm(grads, 0.5)
            grads_and_var = list(zip(grads, var))
            self._rr(tf.reshape(tf.concat([tf.reshape(grad,[-1]) for grad in grads], axis=0),[-1]), 'clipped_gradients');

            _train = trainer.apply_gradients(grads_and_var)
            self._train= U.function(self._get_placeholders(names =[
                "observations",
                "actions",
                "advantages",
                "returns"
